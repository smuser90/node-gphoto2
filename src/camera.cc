...
void GPCamera::Async_CaptureCb(uv_work_t *req, int status) {
  ...
  gp_camera_unref(capture_req->camera); // not sure why this was commented out
                                        // before - seems to be a memory leak
                                        // -EP
  delete capture_req;
}
...
void GPCamera::Async_WaitEvent(uv_work_t *req) {
  ...
  else if (eventType == GP_EVENT_FILE_ADDED) event_req->eventType = "file_added";
  else if (eventType ==
           GP_EVENT_FOLDER_ADDED) event_req->eventType = "folder_added";
  else event_req->eventType = "unknown";
...
Handle<Value>GPCamera::SetConfigValue(const Arguments& args) {
  ...
  if (args[1]->IsString()) {
    ...
    return ThrowException(Exception::TypeError(
                            String::New(
                              "Argument 1 invalid: String, Integer or Float value "
                              "expected")));
  }
  config_req->cameraObject = camera;
  config_req->camera       = camera->getCamera();
